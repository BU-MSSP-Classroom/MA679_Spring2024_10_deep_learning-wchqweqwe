...
beta_range <- seq(-6, 6, length.out = 400)
R_values <- R(beta_range)

plot(beta_range, R_values)
```
...
beta_values <- numeric(iterations + 1)
beta_values[1] <- beta_0

# Perform gradient descent
for (i in 1:iterations) {
  beta_prev <- beta_values[i]
  # Update beta based on the gradient of R at beta_prev
  beta_new <- beta_prev - rho * dR_dbeta(beta_prev)
  beta_values[i + 1] <- beta_new
}

# Compute R(beta) for the updated beta values
R_beta_values <- R(beta_values)

# Plot the original function
{plot(beta_range, R_values, type = "l", col = "blue", 
     main = "Gradient Descent on R(beta) = sin(beta) + beta/10",
     xlab = "beta", ylab = "R(beta)",
     lwd = 2)
grid()

# Add the gradient descent path
points(beta_values, R_beta_values, col = "red", pch = 19)}

# Print the final beta value
print(paste("Final beta:", beta_values[length(beta_values)]))
```
...
l = f(theta|x) = 10 C 4 * theta^4 * (1-theta)^6 
